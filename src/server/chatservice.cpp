...
void ChatService::login(const TcpConnectionPtr &conn,json &js,Timestamp)
{
    ...
    string password = js["password"];  //获取密码 

    User user = _userModel.query(id); //查找 这里要完善usermodel.cpp usermodel.hpp

    if(user.getId() == id && user.getPassword() == password) //查出来了，登录成功 
    {
        ...
        {
            //登录成功，记录用户连接信息，当多个用户登录时，都会操作这个_userConnMap
            {
            ...
            _userModel.updateState(user); //这里要完善usermodel.cpp usermodel.hpp
            ...
            //查询该用户是否有离线消息
            vector<string> vec = _offlineMsgModel.query(id); //查询用户ID
            if(!vec.empty()) //不为空
            {
...
void ChatService::reg(const TcpConnectionPtr &conn,json &js,Timestamp)
{
    ...
    if(_userModel.insert(user)) //插入成功 
    {
         //注册成功
        json response;
...
void ChatService::addFriend(const TcpConnectionPtr &conn,json &js,Timestamp)
{
    ...
    int friendid = js["friendid"].get<int>(); //获取对方的id号 
    {
        lock_guard<mutex> lock(_connMutex);

        auto it = _userConnMap.find(friendid);  //查找对方id号 

        if(it != _userConnMap.end()) //找到了
        {
            //toid在线，转发消息  服务器主动推送消息给friendid用户
            it->second->send(js.dump());
            return;
        }
    }
